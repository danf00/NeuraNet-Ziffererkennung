...
for variant in variants:
    ...
    modell = np.load(f"variants/{variant}/feige1.1.npz")
    ...
    for i in range(9999):
        ...
        Z1, A1, Z2, A2, Z3, A3 = forward(X_train[i], W1, b1, W2, b2, W3, b3)

        if Y_train[i] == np.argmax(A3, axis=1)[0]:
            correct += 1
        else:
            wrong += 1

    print("feige1.1--", variant, ": accuracy: ", correct / (correct + wrong))
